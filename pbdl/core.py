...
class Tag():
	...
	def write_info_mp4(self, data, filepath=None):
		...
		props = get_props()
		for k in data:
			code = self._conv_to_text(props[k])
			f.tags[code] = data[k]
		f.save()
...
class TMDB():
	...
	def _search_movies(self, query):
		return self._get("https://imdb232.p.rapidapi.com/api/search", {"count":200,"type":"MOVIE","q":query})
	...
	def search_episodes(self, imdbid, season):
		if 'tt' not in imdbid:
			query = imdbid
			imdbid = self.get_imdbid(query)
